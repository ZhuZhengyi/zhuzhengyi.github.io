...
impl Solution {
    ...
    pub fn rotate_right(head: Option<Box<ListNode>>, k: i32) -> Option<Box<ListNode>> {
        ...
        let mut head = head;
        //获取截断点上一个结点可变借用
        let mut pre_cut_node_ptr: &mut Box<ListNode> = Solution::get_list_n_node_mut_ref(&mut head, cut_nodes - 1);

        //释放截断节点所有权，转移给new_head
        let mut new_head: Option<Box<ListNode>> = pre_cut_node_ptr.next.take();

        //获取截断链表的最后一个结点可变引用
        let mut last_node_ptr = Solution::get_list_last_node_ref(&mut new_head);
