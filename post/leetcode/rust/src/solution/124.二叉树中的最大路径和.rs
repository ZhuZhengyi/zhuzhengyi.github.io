...
impl Solution {
    ...
    pub fn max_path_sum(root: Option<Rc<RefCell<TreeNode>>>) -> i32 {
        // 以node为端点的最大一边路径和。
        fn max_path_sum_end_with_node(node: &Option<Rc<RefCell<TreeNode>>>, max_sum: &mut i32) -> i32 {
            match node {
                None => 0,
                Some(node) => {
                    let left_sum = cmp::max(0, max_path_sum_end_with_node(&node.borrow().left, max_sum) );
                    let right_sum = cmp::max(0, max_path_sum_end_with_node(&node.borrow().right, max_sum) );
                    let total_path_sum = left_sum + node.borrow().val + right_sum;
                    *max_sum = cmp::max(*max_sum, total_path_sum);
                    // 
